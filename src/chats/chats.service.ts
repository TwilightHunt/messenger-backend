...
export class ChatsService {
  ...
  async send(data) {
    ...
    const chat = userChatsInDB.chats.find((el) => el.receiver === receiver);
  ...
  async getMessage(data) {
    ...
    const chat = receiverChatsInDB.chats.find((el) => el.receiver === senter._id);
  ...
  async getUserChats(uid: string) {
    return await this.userChatsModel.findOne({ user: uid });
  }
